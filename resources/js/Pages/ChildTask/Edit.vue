...
function updateChildTask() {
  router.put(
    `/projects/${props.project.id}/tasks/${props.task.id}/child-tasks/${props.child_task.id}`,
    form
  );
}
        ...
        <form @submit.prevent="updateChildTask">
          ...
          <div class="bg-white p-5 m-5">
            <textarea
              ...
              rows="12"
              class="w-full sm:rounded-lg border-gray-300"
              placeholder="課題の詳細"
            ></textarea>
            <div>
              <label>状態</label>
              ...
              >
                <option
                  v-for="projectUser in props.project_users"
                  :key="projectUser.id"
