              ...
              <table class="table-auto w-full text-left whitespace-no-wrap">
                <thead class="shadow-lg text-green-700">
                  <tr>
                    ...
                    <th
                      class="px-3 py-3 title-font tracking-wider font-medium text-sm text-center"
                    >
                      担当者
                    </th>
                    <th
                ...
                <tbody v-for="task in tasks" :key="task.id">
                  <tr
                    class="border-b border-gray-300 hover:bg-blue-200 text-sm"
                  ...
                  >
                    ...
                    <td class="px-6 py-3 text-center">
                      <div
                        v-if="task.status === '完了'"
                        class="rounded-full p-1 bg-slate-300"
                      >
                        {{ task.status }}
                      </div>
                      <div
                        v-if="task.status === '処理済み'"
                        class="rounded-full p-1 bg-indigo-200"
                      >
                        {{ task.status }}
                      </div>
                      <div
                        v-if="task.status === '未対応'"
                        class="rounded-full p-1 bg-orange-200"
                      >
                        {{ task.status }}
                      </div>
                      <div
                        v-if="task.status === '処理中'"
                        class="rounded-full p-1 bg-green-300"
                      >
                        {{ task.status }}
                      </div>
                    </td>
                    <td class="px-6 py-3 text-center text-white">
                      <div v-if="task.type">
                        <div
                          v-if="task.type.name === 'バグ'"
                          class="rounded-full p-1 bg-red-600"
                        >
                          {{ task.type.name }}
                        </div>
                        <div
                          v-if="task.type.name === '実装'"
                          class="rounded-full p-1 bg-blue-600"
                        >
                          {{ task.type.name }}
                        </div>
                        <div
                          v-if="task.type.name === '改善'"
                          class="rounded-full p-1 bg-pink-600"
                        >
                          {{ task.type.name }}
                        </div>
                        <div
