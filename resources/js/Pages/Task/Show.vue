...
function storeChildTaskGpt() {
  try {
    ...
    router.post(
      `/projects/${props.project.id}/tasks/${props.task.id}/child-tasks/store-gpt`
    );
    // 成功時の処理
  } catch (error) {
    // エラー処理
...
function storeBranchGpt() {
  try {
    // isLoading.value = true; // ローディング開始
    router.post(
      `/projects/${props.project.id}/tasks/${props.task.id}/create-branch-gpt`
    );
    // 成功時の処理
  } catch (error) {
    // エラー処理
...
function storeTaskComment() {
  router.post(`/projects/${props.project.id}/tasks/${props.task.id}/comments`, {
        ...
        <div class="p-5">
          <div class="h-auto bg-white p-10 rounded border border-gray-200">
            ...
            <div>
              ...
              <div
                v-html="props.task.content"
                class="px-5 pt-8 pb-5 text-sm"
              ></div>
              <hr />
            </div>
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td v-if="props.task.type" class="border-b border-gray-300">
                    ...
                    <span
                      v-if="
                        props.task.type.name !== '改善' &&
                        props.task.type.name !== '実装' &&
                        props.task.type.name !== 'バグ'
                      "
          ...
          <div class="flex justify-end my-5">
            <Link
              :href="
                route('projects.tasks.edit', { project: project, task: task })
              "
          ...
          <div class="h-auto mt-10 bg-white p-4 rounded border border-gray-200">
            <div class="flex">
              ...
              <button
                v-if="!props.childTasks || props.childTasks.length === 0"
          ...
          <div class="w-full bg-white my-10 border rounded">
            ...
            <hr />
            <div v-for="comment in props.task.comments" :key="comment.id">
            ...
            <form
              @submit.prevent="storeTaskComment"
