...
const renderChildTaskShow = (childTask) =>
    router.get(`/projects/${props.project.id}/tasks/${childTask.project_task_number.task_number}`)

function storeChildTaskGpt() {
    try {
        // isLoading.value = true; // ローディング開始
        router.post(`/projects/${props.project.id}/tasks/${props.task.taskable.id}/child-tasks/store-gpt`);
        // 成功時の処理
    } catch (error) {
        // エラー処理
...
function storeChildTask() {
    router.post(`/projects/${props.project.id}/tasks/${props.task.taskable.id}/child-tasks`, form)
    // 保存が成功した後にフォームをリセット
    form.user_id = null;
    form.title = null;
    form.content = null;
                ...
                <div class="p-5">
                    <div class="h-auto bg-white p-10 rounded border border-gray-200">
                        <div>
                            <div class="flex">
                                ...
                                <p class="ml-auto">
                                    登録者： {{ props.task.taskable.creator.name }}<br>
                                    登録日時： {{ formatDate(props.task.taskable.created_at) }}
                                </p>
                            </div>
                            <p class="font-semibold px-5 pt-8 pb-5 text-sm">{{ props.task.taskable.title }}</p>
                            <hr>
                        </div>
                        <div>
                            <p class="font-semibold p-5">詳細</p>
                            <div v-html="props.task.taskable.content" class="px-5 pt-8 pb-5 text-sm"></div>
                            <hr>
                        </div>
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td class="border-b border-gray-300 w-80">
                                        <span v-if="props.task.taskable.status === '完了'"
                                            class="rounded-full py-2 px-6 bg-slate-300">
                                            {{ props.task.taskable.status }}
                                        </span>
                                        <span v-if="props.task.taskable.status === '処理済み'"
                                    ...
                                    <td class="border-b border-gray-300 w-80">{{ props.task.taskable.user.name }}</td>
                                </tr>
                                <tr>
                                    <td class="pl-8 text-left border-b border-gray-300 py-8 w-1/6">優先度</td>
                                    <td v-if="props.task.taskable.priority === '高'"
                                        class="text-lg text-red-600 border-b border-gray-300">
                                        {{ props.task.taskable.priority }}
                                    </td>
                                    <td v-if="props.task.taskable.priority === '中'"
                                        class="text-lg text-green-500 border-b border-gray-300">
                                        {{ props.task.taskable.priority }}
                                    </td>
                                    <td v-if="props.task.taskable.priority === '低'"
                                        class="text-lg text-blue-500 border-b border-gray-300">
                                        {{ props.task.taskable.priority }}
                                    </td>
                                    <td v-if="!props.task.taskable.priority"
                                        class="text-lg text-red-600 border-b border-gray-300"></td>
                                    <td class="w-1/12"></td>
                                    <td class="py-3 pl-8 text-left border-b border-gray-300">種別</td>
                                    <td v-if="props.task.taskable.type" class="border-b border-gray-300">
                                        <span v-if="props.task.taskable.type.name === 'バグ'"
                                            class="rounded-full py-2 px-6 bg-red-600 text-white text-xs">
                                            {{ props.task.taskable.type.name }}
                                        </span>
                                        <span v-if="props.task.taskable.type.name === '実装'"
                                            class="rounded-full py-2 px-6 bg-blue-600 text-white text-xs">
                                            {{ props.task.taskable.type.name }}
                                        </span>
                                        <span v-if="props.task.taskable.type.name === '改善'"
                                            class="rounded-full py-2 px-6 bg-pink-600 text-white text-xs">
                                            {{ props.task.taskable.type.name }}
                                        </span>
                                        <span
                                            v-if="props.task.taskable.type.name !== '改善' && props.task.taskable.type.name !== '実装' && props.task.taskable.type.name !== 'バグ'"
                                            class="rounded-full py-2 px-6 bg-slate-500 ml-10 text-white text-xs">
                                            {{ props.task.taskable.type.name }}
                                        </span>
                                    </td>
                                    <td v-if="!props.task.taskable.type" class="border-b border-gray-300"></td>
                                </tr>
                                <tr>
                                    <td class="pl-8 text-left border-b border-gray-300 py-8">ブランチ名</td>
                                    <td class="border-b border-gray-300">
                                        <span class="">{{ props.task.taskable.branch_name }}</span>
                                        <button v-if="!props.task.taskable.branch_name" @click="storeBranchGpt"
                                ...
                                <tr>
                                    ...
                                    <td class="border-b border-gray-300 text-base">{{
                                        formatDate(props.task.taskable.start_date) }}
                                    </td>
                                    <td class="w-1/12"></td>
                                    <td class="pl-8 text-left border-b border-gray-300">終了日</td>
                                    <td class="border-b border-gray-300 text-base">{{
                                        formatDate(props.task.taskable.end_date) }}
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                    ...
                    <div class="h-auto mt-10 bg-white p-4 rounded border border-gray-200">
                        <div class="flex">
                            ...
                            <button
                                v-if="!props.childTasks[0] && props.task.taskable_type === 'App\Models\ChildTask'"
                                @click="storeChildTaskGpt"
